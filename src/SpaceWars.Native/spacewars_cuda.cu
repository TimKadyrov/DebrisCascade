// SpaceWars CUDA engine: batch J2-secular propagation + time-averaged spatial-density
// accumulation over the full nail + catalog population. The device propagator mirrors
// SpaceWars.Core.OrbitalElements.PositionAt so GPU results validate against the C# ref.
//
// Build (from an MSVC x64 dev shell):
//   nvcc -O3 -arch=sm_120 --shared -o spacewars_cuda.dll spacewars_cuda.cu
...
// SpaceWars.Interop.ObjElem field order exactly (10 sequential doubles).
struct ObjElem {
